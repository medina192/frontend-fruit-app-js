...
const LoginScreen = ({ navigation }) => {
    ...
    const heightForm = windowHeight * 0.45;

    const animationImage = useRef(new Animated.Value(0)).current;
    const animationForm = useRef(new Animated.Value( heightForm )).current;

    const [emailInpurIsFocused, setEmailInpurIsFocused] = useState(false);
    const [passwordInpurIsFocused, setPasswordInpurIsFocused] = useState(false);
    ...
    const hideForm = (durationAnimation = 800) => {
    ...
    const LogIn = async() => {
        ...
        {
            try {
                const r = await Auth.signIn(emailInputValue.toLowerCase(), passwordInputValue)
                console.log("hi", r)
                hideForm(1)
                setEmailInputValue("");
